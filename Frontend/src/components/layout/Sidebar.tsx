...
export const Sidebar: React.FC<SidebarProps> = ({ isOpen, onClose }) => {
  ...
  return (
    <>
      ...
      <aside
        className={`fixed left-0 top-0 h-full bg-white text-slate-800 border-r border-slate-200 transition-all duration-300 z-[70] ${
          isOpen ? "w-64" : "w-16"
        } ${isOpen ? "translate-x-0" : "-translate-x-full lg:translate-x-0"}`}
      >
        <div className="flex flex-col h-full">
          <div className="p-4 flex items-center justify-between border-b border-slate-200 min-h-[64px]">
            {isOpen ? (
              <>
                <span className="font-bold text-xl tracking-tight text-blue-600 italic">PBBET</span>
                <button onClick={onClose} className="lg:hidden text-slate-500 hover:text-slate-800 transition-colors">
                   <X size={24} />
                </button>
              </>
            ) : (
              <div className="w-8 h-8 rounded bg-blue-500/10 flex items-center justify-center mx-auto">
          ...
          <nav className="flex-1 overflow-y-auto py-4 no-scrollbar">
            <ul className="space-y-1 px-2">
              {menuItems.map((item, idx) => (
                <li key={idx}>
                  <button
                    className={`w-full flex items-center px-3 py-2.5 gap-4 rounded-[5px] transition-all duration-200 hover:bg-slate-100 hover:text-slate-900 ${
                      item.active
            ...
            <div className="mt-8 px-4">
               {isOpen && <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-4 px-2">{BANGLA_TEXT.SOCIAL_MEDIA}</p>}
               <div className={`flex ${isOpen ? "flex-col space-y-1" : "flex-col items-center space-y-4"}`}>
                  {socialIcons.map((social, idx) => (
                    <button key={idx} className="flex items-center gap-4 px-2 py-2 text-slate-500 hover:text-slate-850 transition-colors">
                      {social.icon}
